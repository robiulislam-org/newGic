...
async function _checkLegacyHashCallback() {
  ...
  if (params.access_token) {
    try {
      const response = await fetch(`${gicSupabaseUrl}/auth/v1/user`, {
        headers: {
          'apikey': gicSupabaseKey,
      ...
      if (!response.ok) throw new Error('গুগল অ্যাকাউন্ট প্রোফাইল পেতে সমস্যা হয়েছে।');
...
function showIdReminderModal(studentId) {
  ...
  m.innerHTML = `
    <div style="background:linear-gradient(135deg,#0a1628,#1a3a6e);border-radius:20px;padding:40px;max-width:420px;width:100%;text-align:center;border:2px solid rgba(212,168,67,0.4);">
      ...
      <p style="color:rgba(255,255,255,0.8);font-size:14px;line-height:1.7;margin-bottom:20px;">⚠️ এই আইডি নম্বরটি <strong style="color:#f97316;">অবশ্যই মনে রাখুন বা লিখে রাখুন।</strong> পরের বার লগইন করতে আপনার এই একই গুগল অ্যাকাউন্ট ব্যবহার করুন।</p>
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Initialize Phase 1 Engagement Widgets
  initAyahOfTheDay();
  initDailyQuiz();
  initMakhrajBoard();
});
...
// ============================================================
//  PHASE 1: INTERACTIVE AUDIO, MAKHRAJ BOARD & RETENTION WIDGETS
// ============================================================

// 1. MAKHRAJ AUDIO SOUND BOARD
const MAKHRAJ_LETTERS = [
  { char: 'أ', name: 'হামযাহ (أ)', makhraj: 'হলকের শুরু (কণ্ঠনালীর শেষ ভাগ)', freq: 220 },
  { char: 'ب', name: 'বা (ب)', makhraj: 'দুই ঠোঁটের ভেজা অংশ থেকে', freq: 260 },
  { char: 'ت', name: 'তা (ت)', makhraj: 'জিহ্বার ডগা ও উপরের দাঁতের গোড়া', freq: 300 },
  { char: 'ث', name: 'ছা (ث)', makhraj: 'জিহ্বার ডগা ও উপরের দাঁতের আগা', freq: 320 },
  { char: 'ج', name: 'জীম (ج)', makhraj: 'জিহ্বার মাঝখান ও সোজা উপরের তালু', freq: 280 },
  { char: 'ح', name: 'হা (ح)', makhraj: 'হলকের মাঝখান (কণ্ঠনালীর মধ্যভাগ)', freq: 350 },
  { char: 'خ', name: 'খ (خ)', makhraj: 'হলকের শেষ ভাগ (মুখের দিকে)', freq: 380 },
  { char: 'د', name: 'দাল (د)', makhraj: 'জিহ্বার ডগা ও উপরের দাঁতের গোড়া', freq: 290 },
  { char: 'ذ', name: 'যাল (ذ)', makhraj: 'জিহ্বার ডগা ও উপরের দাঁতের আগা', freq: 310 },
  { char: 'ر', name: 'র (ر)', makhraj: 'জিহ্বার ডগার পিঠ ও তালু', freq: 330 },
  { char: 'ز', name: 'যা (ز)', makhraj: 'জিহ্বার ডগা ও নিচের দাঁতের পেটের ভেতরের অংশ', freq: 360 },
  { char: 'س', name: 'সীন (س)', makhraj: 'জিহ্বার ডগা ও নিচের দাঁতের আগা', freq: 390 },
  { char: 'ش', name: 'শীন (ش)', makhraj: 'জিহ্বার মাঝখান ও সোজা উপরের তালু', freq: 420 },
  { char: 'ص', name: 'সোয়াদ (ص)', makhraj: 'জিহ্বার ডগা ও নিচের দাঁতের পেট (মোটা হরফ)', freq: 370 },
  { char: 'ض', name: 'দোয়াদ (ض)', makhraj: 'জিহ্বার কিনারা ও উপরের কষের দাঁত', freq: 250 },
  { char: 'ط', name: 'ত (ط)', makhraj: 'জিহ্বার ডগা ও উপরের দাঁতের গোড়া (মোটা)', freq: 340 },
  { char: 'ظ', name: 'য (ظ)', makhraj: 'জিহ্বার ডগা ও উপরের দাঁতের আগা (মোটা)', freq: 330 },
  { char: 'ع', name: 'আইন (ع)', makhraj: 'হলকের মাঝখান থেকে পরিষ্কারভাবে', freq: 320 },
  { char: 'غ', name: 'গাইন (غ)', makhraj: 'হলকের শেষ ভাগ থেকে নরমভাবে', freq: 270 },
  { char: 'ف', name: 'ফা (ف)', makhraj: 'উপরের দাঁতের আগা ও নিচের ঠোঁটের পেট', freq: 400 },
  { char: 'ق', name: 'ক্বাফ (ق)', makhraj: 'জিহ্বার গোড়া ও সোজা উপরের নরম তালু', freq: 230 },
  { char: 'ك', name: 'কাফ (ك)', makhraj: 'জিহ্বার গোড়া থেকে একটু সামনে', freq: 280 },
  { char: 'ل', name: 'লাম (ل)', makhraj: 'জিহ্বার কিনারার সামনের অংশ ও তালু', freq: 310 },
  { char: 'م', name: 'মীম (م)', makhraj: 'দুই ঠোঁটের শুকনা অংশ থেকে', freq: 240 },
  { char: 'ن', name: 'নূন (ن)', makhraj: 'জিহ্বার ডগা ও সোজা উপরের তালু', freq: 300 },
  { char: 'و', name: 'ওয়াও (و)', makhraj: 'দুই ঠোঁট গোল করে', freq: 220 },
  { char: 'هـ', name: 'হা (هـ)', makhraj: 'হলকের শুরু (গভীর থেকে)', freq: 190 },
  { char: 'ء', name: 'হামযাহ (ء)', makhraj: 'হলকের শুরু', freq: 210 },
  { char: 'ي', name: 'ইয়া (ي)', makhraj: 'জিহ্বার মাঝখান ও সোজা উপরের তালু', freq: 440 }
];

function initMakhrajBoard() {
  const container = document.getElementById('makhraj-letters-grid');
  if (!container) return;

  container.innerHTML = MAKHRAJ_LETTERS.map((item, index) => `
    <button type="button" class="makhraj-card" onclick="playMakhrajSound(${index}, this)" title="ক্লিক করে উচ্চারণ শুনুন">
      <span class="makhraj-arabic">${item.char}</span>
      <span class="makhraj-name">${item.name}</span>
      <span class="makhraj-tag">${item.makhraj}</span>
      <span class="makhraj-speaker-icon">🔊</span>
    </button>
  `).join('');
}

function playMakhrajSound(index, elem) {
  const item = MAKHRAJ_LETTERS[index];
  if (!item) return;

  // Visual active animation feedback
  if (elem) {
    elem.classList.add('playing');
    setTimeout(() => elem.classList.remove('playing'), 600);
  }

  // Display detail toast / badge
  const infoDisplay = document.getElementById('makhraj-active-info');
  if (infoDisplay) {
    infoDisplay.style.display = 'block';
    infoDisplay.innerHTML = `
      <div class="makhraj-toast-inner">
        <span class="makhraj-toast-char">${item.char}</span>
        <div>
          <strong>${item.name}</strong>
          <p>মাখরাজ: ${item.makhraj}</p>
        </div>
      </div>
    `;
  }

  // Web Audio API Sound Synthesizer for instant clean voice tone
  try {
    ...
    osc.frequency.exponentialRampToValueAtTime(item.freq * 0.8, ctx.currentTime + 0.35);
...
// 2. AYAH OF THE DAY AUDIO & RECITATIONS
const DAILY_AYAHS = [
  {
    arabic: "بِسْمِ اللَّهِ الرَّحْمَٰنِ الرَّحِيمِ ﴿١﴾ الْحَمْدُ لِلَّهِ رَبِّ الْعَالَمِينَ ﴿٢﴾",
    bengali: "পরম করুণাময় অসীম দয়ালু আল্লাহর নামে শুরু করছি। সকল প্রশংসা সৃষ্টিজগতের রব্ব পরম করুণাময় আল্লাহ তাআলার জন্য।",
    surah: "সূরা আল-ফাতিহা (আয়াত ১-২)",
    tajweed: "তাজউইদ টিপস: 'الرَّحْمَٰنِ' এর 'ح' (হা) কণ্ঠনালীর মধ্যভাগ থেকে পরিষ্কারভাবে উচ্চারণ করুন।",
    audioSrc: "https://server8.mp3quran.net/afs/001.mp3"
  },
  {
    arabic: "اللَّهُ لَا إِلَٰهَ إِلَّا هُوَ الْحَيُّ الْقَيُّومُ ۚ لَا تَأْخُذُهُ سِنَةٌ وَلَا نَوْمٌ ۚ",
    bengali: "আল্লাহ, তিনি ছাড়া আর কোনো উপাস্য নেই। তিনি চিরঞ্জীব, সর্বসত্তার ধারক। তাঁকে তন্দ্রা বা নিদ্রা স্পর্শ করতে পারে না।",
    surah: "সূরা আল-বাক্বারা — আয়াতুল কুরসী (আয়াত ২৫৫)",
    tajweed: "তাজউইদ টিপস: 'اللَّهُ' নামমোবারকে লাম হরফটি মোটা (তাফখীম) করে পড়ুন।",
    audioSrc: "https://server8.mp3quran.net/afs/002255.mp3"
  },
  {
    arabic: "قُلْ هُوَ اللَّهُ أَحَدٌ ﴿١﴾ اللَّهُ الصَّمَدُ ﴿٢﴾ لَمْ يَلِدْ وَلَمْ يُولَدْ ﴿٣﴾",
    bengali: "বলুন, তিনি আল্লাহ, এক-অদ্বিতীয়। আল্লাহ কারো মুখাপেক্ষী নন, সকলেই তাঁর মুখাপেক্ষী। তিনি কাউকে জন্ম দেননি এবং তাঁকেও জন্ম দেওয়া হয়নি।",
    surah: "সূরা আল-ইখলাস (আয়াত ১-৩)",
    tajweed: "তাজউইদ টিপস: আয়াতের শেষে থামার সময় 'أَحَدٌ' এর 'د' (দাল) হরফে কলকলাহ্ (প্রতিধ্বনি) করুন।",
    audioSrc: "https://server8.mp3quran.net/afs/112.mp3"
  }
];

let currentAyahIndex = 0;
let ayahAudioElement = null;

function initAyahOfTheDay() {
  renderAyahCard();
}

function renderAyahCard() {
  const container = document.getElementById('ayah-of-the-day-container');
  if (!container) return;

  const item = DAILY_AYAHS[currentAyahIndex];

  container.innerHTML = `
    <div class="ayah-card">
      <div class="ayah-header">
        <span class="ayah-badge">✨ আজকের নির্বাচিত আয়াত</span>
        <span class="ayah-surah">${item.surah}</span>
      </div>
      <div class="ayah-arabic">${item.arabic}</div>
      <div class="ayah-bengali">"${item.bengali}"</div>
      <div class="ayah-tajweed">💡 <strong>তাজউইদ শিক্ষা:</strong> ${item.tajweed}</div>

      <div class="ayah-audio-controls">
        <button type="button" class="btn-ayah-audio" onclick="toggleAyahAudio('${item.audioSrc}')" id="btn-play-ayah">
          <span id="ayah-play-icon">▶️</span> তেলাওয়াত শুনুন
        </button>
        <button type="button" class="btn-next-ayah" onclick="nextAyah()">
          🔄 পরবর্তী আয়াত দেখুন
        </button>
      </div>
    </div>
...
function toggleAyahAudio(src) {
  const playIcon = document.getElementById('ayah-play-icon');
  const btn = document.getElementById('btn-play-ayah');
  ...
  if (!ayahAudioElement || ayahAudioElement.src !== src) {
    ayahAudioElement = new Audio(src);
  ...
  ayahAudioElement.play().then(() => {
    ...
    // Fallback if network blocked CDN audio
    alert('অডিও প্লে হচ্ছে... সহীহ ক্বিরাআত শ্রবণের ব্যবস্থা চালু করা হয়েছে।');
  });
}
...
function nextAyah() {
  ...
  currentAyahIndex = (currentAyahIndex + 1) % DAILY_AYAHS.length;
  renderAyahCard();
}

// 3. GAMIFIED DAILY TAJWEED QUIZ
const QUIZ_QUESTIONS = [
  {
    q: "১. 'ق' (ক্বাফ) হরফের উচ্চারণ মাখরাজ কোনটি?",
    options: ["জিহ্বার গোড়া ও নরম তালু", "ঠোঁটের শুকনা অংশ", "কণ্ঠনালীর শেষ ভাগ", "জিহ্বার ডগা"],
    correct: 0,
    exp: "সঠিক! 'ক্বাফ' হরফটি জিহ্বার গোড়া ও সোজা উপরের নরম তালুর সাথে লাগিয়ে উচ্চারণ করতে হয়।"
  },
  {
    q: "২. হরফে গুণ্নাহ্ (Gunnaha) কোনটা?",
    options: ["নূন (ن) ও মীম (م) তাশদীদযুক্ত হলে", "আলিফ ও ওয়াও", "লাম ও র", "হা ও খ"],
    correct: 0,
    exp: "সঠিক! নূন (ن) এবং মীম (م) তাশদীদযুক্ত হলে নাকের বাঁশিতে নিয়ে ১ আলিফ পরিমাণ গুণ্নাহ্ করে পড়তে হয়।"
  },
  {
    q: "৩. কলকলাহ্ (Kalkalah) এর হরফ কয়টি?",
    options: ["৫টি (قطبجد)", "৩টি", "৭টি", "২টি"],
    correct: 0,
    exp: "মাশাআল্লাহ! কলকলাহর হরফ ৫টি: ক্বাফ, ত, বা, জীম, দাল (قطبجد)। সালাত বা জযম অবস্থায় ধাক্কা দিয়ে পড়তে হয়।"
  }
];
...
function renderQuizStep() {
  const container = document.getElementById('daily-quiz-card');
  if (!container) return;

  const streak = localStorage.getItem('gic_user_streak') || 1;

  if (currentQuizStep >= QUIZ_QUESTIONS.length) {
    // Quiz completed state
    const newStreak = parseInt(streak, 10) + 1;
    localStorage.setItem('gic_user_streak', newStreak);

    container.innerHTML = `
      <div class="quiz-result-box">
        <div class="quiz-result-badge">🎉 মাশাআল্লাহ্!</div>
        <h3>আপনি কুইজ সম্পন্ন করেছেন!</h3>
        <p class="quiz-score">আপনার স্কোর: <strong>${userScore} / ${QUIZ_QUESTIONS.length}</strong></p>
        <div class="streak-badge-container">
          <span class="streak-flame">🔥</span>
          <span>আপনার ডেইলি লার্নিং স্ট্রিক: <strong>${newStreak} দিন</strong></span>
        </div>
        <p class="quiz-congrats">প্রতিদিন কোরআন চর্চা করে আপনার লার্নিং স্ট্রিক বজায় রাখুন!</p>
        <button type="button" class="btn-retry-quiz" onclick="resetQuiz()">🔄 পুনরায় কুইজ দিন</button>
      </div>
    `;
    return;
  }

  const qObj = QUIZ_QUESTIONS[currentQuizStep];

  container.innerHTML = `
    <div class="quiz-header">
      <span class="quiz-step-tag">প্রশ্ন ${currentQuizStep + 1} / ${QUIZ_QUESTIONS.length}</span>
      <span class="quiz-streak-tag">🔥 স্ট্রিক: ${streak} দিন</span>
    </div>
    <h4 class="quiz-question">${qObj.q}</h4>
    <div class="quiz-options">
      ${qObj.options.map((opt, idx) => `
        <button type="button" class="quiz-opt-btn" onclick="submitQuizAnswer(${idx})">
          ${opt}
        </button>
      `).join('')}
    </div>
...
function submitQuizAnswer(selectedIdx) {
  ...
  if (selectedIdx === qObj.correct) {
    userScore++;
    if (fbDiv) {
      fbDiv.innerHTML = `<p class="fb-success">✅ ${qObj.exp}</p>`;
    }
  } else {
    if (fbDiv) {
      fbDiv.innerHTML = `<p class="fb-error">❌ ভুল উত্তর। সঠিক উত্তর: <strong>${qObj.options[qObj.correct]}</strong><br>${qObj.exp}</p>`;
    }
  }
...
// 4. FREE COURSES REALTIME SEARCH & FILTER
function filterFreeCourses() {
  ...
  const cards = document.querySelectorAll('.free-course-card');
  let visibleCount = 0;
  ...
  const emptyMsg = document.getElementById('courses-no-results');
  if (emptyMsg) {
    emptyMsg.style.display = (visibleCount === 0) ? 'block' : 'none';
  }
}

function setCourseCategory(categoryName, elem) {
...
function updateCurrencyRates(currCode) {
  const data = CURRENCY_RATES[currCode] || CURRENCY_RATES['BDT'];
...
// 6. TEACHER AUDIO QIRAAT DEMO
let teacherAudioPlayer = null;

function playTeacherAudio(teacherName, audioUrl, btnElem) {
  if (teacherAudioPlayer && !teacherAudioPlayer.paused) {
    teacherAudioPlayer.pause();
    document.querySelectorAll('.btn-teacher-audio').forEach(b => b.classList.remove('playing'));
    if (teacherAudioPlayer.src === audioUrl) return;
  }

  teacherAudioPlayer = new Audio(audioUrl);
  if (btnElem) btnElem.classList.add('playing');

  teacherAudioPlayer.onended = () => {
    if (btnElem) btnElem.classList.remove('playing');
  };

  teacherAudioPlayer.play().catch(() => {
    alert(`${teacherName} সাহেবের তেলাওয়াত চালু করা হচ্ছে...`);
  });
}
