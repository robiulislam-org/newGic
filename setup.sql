...
create table if not exists analytics_events (
  ...
  student_email      text default '',      -- Gmail if logged in (or previously logged in on this device)
  device_fingerprint text default '',      -- Stable device hash (identifies returning visitors without login)
  visitor_id         text default '',      -- Persistent localStorage ID per device
  device_type        text default '',      -- Mobile / Tablet / Desktop
  browser            text default '',      -- Chrome / Firefox / Safari / Edge
  os                 text default '',      -- Windows / Android / iOS / macOS
...
-- Safe migration: add any missing columns to existing table
do $$
begin
  ...
  if not exists (select 1 from information_schema.columns where table_name='analytics_events' and column_name='device_fingerprint') then
    alter table analytics_events add column device_fingerprint text default '';
  end if;
  if not exists (select 1 from information_schema.columns where table_name='analytics_events' and column_name='visitor_id') then
    alter table analytics_events add column visitor_id text default '';
  end if;
